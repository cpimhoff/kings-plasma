...
function StoredDecks() {
    ...
    const allCardDefsById = useMemo(() => Object.assign({}, ...cardLibrary.map((cardDef) => ({
        [cardDef.typeId]: cardDef,
    }))), [cardLibrary]);
    const handleLoadDeck = useCallback((storedDeck: StoredDeck) => {
    ...
    return (
        <div style={{ '--player-color': colorCssValue } as CSSProperties}>
            <div className="flex">
                <Button disabled={!isValidDeck} onClick={handleSaveCurrentDeck}>Save current deck</Button>
                <Input
                    className="w-80"
